...
class Transformer_Ranking(nn.Module):
    ...
    def __init__(self, W, T, D_MODEL, N_HEAD, ENC_LAYERS, DEC_LAYERS, D_FF, REL_EMB, DROPOUT,TPP_EMB, INDEX, tpp_batch_size, tpp_num_epoch, datatype,USE_POS_ENCODING = False, USE_GRAPH = False, HYPER_GRAPH = True, USE_KG = True, NUM_NODES = 87, config=None, ENCODER_LAYER = 'lstm', USE_RELATION_KG = False):
        super(Transformer_Ranking, self).__init__()
        ...
        self.TPP_EMB = TPP_EMB
        self.INDEX = INDEX
        self.datatype = datatype
        self.tpp_batch_size = tpp_batch_size
        self.tpp_num_epoch = tpp_num_epoch
        ...
        self.USE_RELATION_KG = USE_RELATION_KG 
        ...
        self.pred = nn.Linear((D_MODEL+(SEC_EMB*n)+ self.TPP_EMB)*NUM_NODES, NUM_NODES) #Concat at the Convolution stage 
        self.tpp_pred = nn.Linear((D_MODEL+(SEC_EMB*n)+ self.TPP_EMB)*NUM_NODES, NUM_NODES) #Concat at the Convolution stage
        ...
        if self.use_graph == 'hgat':
            ...
            self.conv1 = HEATConv(self.D_MODEL + self.TPP_EMB, 8, config['num_node_type'], config['relation_total'], self.REL_EMB, self.REL_EMB, SEC_EMB*2, 4, dropout=0.1)            
            self.conv2 = HEATConv(8*4, self.REL_EMB, config['num_node_type'], config['relation_total'], self.REL_EMB, self.REL_EMB, SEC_EMB*2, 4, dropout=0.1)

            self.lin = nn.Linear(self.REL_EMB*4, SEC_EMB*2)
            self.lin2 = nn.Linear(SEC_EMB*2, self.REL_EMB)
            self.num_rel = config['relation_total']
            self.rel_type_embeddings = nn.Embedding(config['relation_total']+2, self.REL_EMB)
            self.month_embeddings   = nn.Embedding(13, self.REL_EMB, padding_idx=0)
            self.day_embeddings     = nn.Embedding(32, self.REL_EMB, padding_idx=0)
            self.hour_embeddings    = nn.Embedding(25, self.REL_EMB, padding_idx=0)
            self.minutes_embeddings = nn.Embedding(61, self.REL_EMB, padding_idx=0)
            self.sec_embeddings     = nn.Embedding(61, self.REL_EMB, padding_idx=0)
        ...
        self.ent_embeddings = nn.Embedding(config['entity_total']+2, D_MODEL + self.TPP_EMB)

        self.num_nodes = config['entity_total']+2
        
        self.tpp_rel_layer = nn.Sequential(
            nn.Linear(self.REL_EMB + self.TPP_EMB, self.REL_EMB))

  
    def forward(self, xb, ticker_ids, phase, node_to_continous, continous_to_node_type, continous_to_edge_type, yb=None, graph=None, kg=None, tkg=None, relation_graph = None):
        kg_loss = 0
        ...
        if self.INDEX == 'sp500':
            file = '../Stock_KG_TPP/TPP/HPP_files/nasdaq100/{}/HHP_phase-{}_{}_{}_{}_{}/'.format(self.datatype, phase, self.tpp_batch_size, self.tpp_num_epoch, self.datatype, self.TPP_EMB) + 'result.pkl'
        else:
            file = '../Stock_KG_TPP/TPP/HPP_files/{}/{}/HHP_phase-{}_{}_{}_{}_{}/'.format(self.INDEX, self.datatype, phase, self.tpp_batch_size, self.tpp_num_epoch, self.datatype, self.TPP_EMB) + 'result.pkl'
        
        with open(file, 'rb') as f:
            tpp_data = pickle.load(f)
            node_embeddings = tpp_data[0]  
            relation_embeddings = tpp_data[1] 

        # Convert the embeddings to a torch tensor
        tpp_node_embeddings = torch.tensor(node_embeddings)
        tpp_node_embeddings = tpp_node_embeddings.to(x.device)
        
        tpp_relation_embeddings = torch.tensor(relation_embeddings) 
        tpp_relation_embeddings = tpp_relation_embeddings.to(x.device)
               
        tpp_stock_embeddings = torch.stack([tpp_node_embeddings[node_to_continous[idx.item()]] for idx in tkg[4]])
        
        tpp_stock_embeddings = tpp_stock_embeddings.unsqueeze(dim=0)
        x =  torch.cat((x, tpp_stock_embeddings), dim=2)
        ...
        if self.use_graph == 'hgat':
            ...
            tpp_relation_emb = torch.stack([torch.tensor(relation_embeddings[idx.item()-1]) for idx in tkg[1]])
        
            tpp_relation_embed = tpp_relation_emb.to(rel_emb.device)
    
            rel_emb =  torch.cat((rel_emb, tpp_relation_embed), dim=1)
            
            rel_emb = self.tpp_rel_layer(rel_emb)
