...
class CustomDataLoader:
    def __init__(self, path, filename, node_type_to_continuous, edge_type_to_continuous, node_to_continuous, train_test=-1, nbr_size=1, neg_size=1, delim=',', num_edge_types=4, sample_type='cut_off'):
        self.graph = {}
        self.nodes = {}
        ...
        self.node_type_to_continuous = node_type_to_continuous
        self.edge_type_to_continuous = edge_type_to_continuous
        self.node_to_continuous = node_to_continuous
        

        with open(filename) as rf:
            for line in rf:
                sid, s_type, tid, t_type, e_type, timestamp = [int(float(x)) for x in line.strip().split(delim)]
                sid = self.node_to_continuous[sid]
                tid = self.node_to_continuous[tid]
                s_type = self.node_type_to_continuous[s_type]
                t_type = self.node_type_to_continuous[t_type]
                e_type = self.edge_type_to_continuous[e_type]
                
                self.nodes.setdefault(s_type, {}).setdefault(sid, 0)
                self.nodes[s_type][sid] += 1
                self.node_ids_dict[sid] = 1
                self.node_ids_dict[tid] = 1
        ...
        # print("max_d_time:", self.max_d_time, "min_d_time:", self.min_d_time)
        # print("id_map_e_type:", self.data_size, "node_ids_dict:", self.node_size)
        # print("id_map_sid:", len(self.id_map_sid), "id_map_s_type:", len(self.id_map_s_type), "id_map_tid:", len(self.id_map_tid), "id_map_t_type:", len(self.id_map_t_type), "id_map_e_type:", len(self.id_map_e_type), "id_map_time:", len(self.id_map_time))
    ...
    def importance_sampler(self, ids, p):
        ...
        sum_uniq_p = np.dot(p, id_matrix).reshape(-1)
        sum_uniq_q = sum_uniq_p ** 2
        
        delta = 1e-10
        norm_q = sum_uniq_q / (np.sum(sum_uniq_q) + delta)
        norm_q = norm_q / norm_q.sum()
        ...
        weight = np.multiply((sum_uniq_p / norm_q + delta)[sp_ids], sp_counts * 1.0 / self.nbr_size)
    ...
    def cutoff_sampler(self, ids, p):
        if self.nbr_size == 0:
            ...
            return [','.join(ids.astype(str)[len(ids) - self.nbr_size:]), ','.join(np.array(p).astype(str)[len(ids) - self.nbr_size:])]